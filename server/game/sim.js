...
export class Sim {
  ...
  _killEnemy(e, ownerPid) {
    ...
    let score = e.type === 'boss' ? SCORE.boss : e.type === 'midboss' ? SCORE.midboss : e.type === 'medium' ? SCORE.medium : e.isRed ? SCORE.redFormation : SCORE.small;
  ...
  continueRun() {
    ...
    for (const p of this.players.values()) {
      ...
      // respawnPlayer() clears looping/loop-timer/shotCooldown/held-input state left over from the
      // moment of death, so a continue doesn't need to repeat that here.
